...
public interface DiscountPolicyService {
    ...
    /** PK 단건 조회 (없으면 404) */
    DiscountPolicyResponse get(Long policyId);

    /** 등록 */
    DiscountPolicyResponse create(DiscountPolicyRequest request);

    /** 수정 (active_yn 토글도 이 메서드로 처리 - 별도 삭제 API 없음) */
    DiscountPolicyResponse update(Long policyId, DiscountPolicyRequest request);
}
